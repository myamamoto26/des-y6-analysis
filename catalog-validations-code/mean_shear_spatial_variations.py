...
def plot_shear_variations_stacked_ccd(x_side, y_side, ccdres, jk=False, jc=None):
    ...
    def stack_CCDs(ccdres, name, x_side, y_side):
        ...
        if jk:
            ...
            # plt.hist(mean_g1[0].flatten(), bins=200, range=(-0.05,0.05))
            # plt.xlabel(r'$<e_{1}>$')
            ...
            sig = np.nanstd(mean_g1[0]) / np.sqrt(len(mean_g1[0].size))
            ...
            sig = np.nanstd(mean_g2[0]) / np.sqrt(len(mean_g2[0].size))
            ...
            sig = np.nanstd(mean_g1[1]) / np.sqrt(len(mean_g1[1].size))
            ...
            sig = np.nanstd(mean_g2[1]) / np.sqrt(len(mean_g2[1].size))
