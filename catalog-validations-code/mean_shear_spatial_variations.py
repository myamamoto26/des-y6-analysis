...
def plot_shear_variations_stacked_ccd(x_side, y_side, ccdres, jk=False, jc=None):
    ...
    def stack_CCDs(ccdres, name, x_side, y_side):
        ...
        if jk:
            ...
            plt.hist(mean_g1[0].flatten(), bins=20, range=(-1.0,1.0))
            plt.xlabel(r'$<e_{1}>$')
            ...
            sig2 = np.std(mean_g1[0][np.where(~np.isnan(mean_g1[0]))])
            print(mean, sig, sig2)
