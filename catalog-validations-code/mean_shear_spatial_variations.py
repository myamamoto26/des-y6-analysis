...
def plot_shear_variations_stacked_ccd(x_side, y_side, ccdres, jk=False, jc=None):
    ...
    def stack_CCDs(ccdres, name, x_side, y_side):
        ...
        if jk:
            ...
            mean = np.mean(mean_g1[0])
            sig = np.std(mean_g1[0])
            mesh = ax1[0,0].pcolormesh(X,Y,mean_g1[0], vmin=mean-3*sig, vmax=mean+3*sig, cmap=cmap)
            ...
            mean = np.mean(mean_g2[0])
            sig = np.std(mean_g2[0])
            mesh = ax1[0,1].pcolormesh(X,Y,mean_g2[0], vmin=mean-3*sig, vmax=mean+3*sig, cmap=cmap)
            ...
            mean = np.mean(mean_g1[1])
            sig = np.std(mean_g1[1])
            mesh = ax1[1,0].pcolormesh(X,Y,mean_g1[1], vmin=mean-3*sig, vmax=mean+3*sig, cmap=cmap)
            ...
            mean = np.mean(mean_g2[1])
            sig = np.std(mean_g2[1])
            mesh = ax1[1,1].pcolormesh(X,Y,mean_g2[1], vmin=mean-3*sig, vmax=mean+3*sig, cmap=cmap)
