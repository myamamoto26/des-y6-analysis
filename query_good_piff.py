    ...
    if sys.argv[1] == 'mdet':
        ...
        for band in bands:
            ...
            with open('/data/des70.a/data/masaya/metadetect/mdet_'+band+'_ccdnum_expnum_v1.pickle', 'rb') as handle:
                ccdnum_expnum = pickle.load(handle)
            remove_list = np.delete(tilenames, [tilenames.index(t) for t in list(ccdnum_expnum.keys())])
            tilenames = np.delete(tilenames, [tilenames.index(t) for t in remove_list])
            tile_unique_expnum = [np.unique(ccdnum_expnum[tilename]['expnum']) for tilename in tilenames]
            unique_expnum = np.unique(np.concatenate(tile_unique_expnum, axis=0))
            query = """
            select 
                i.expnum,
                i.ccdnum,
            ...
            ################################################################
            out_fname = '/data/des70.a/data/masaya/metadetect/'+band+'_exposure_field_centres_v1.fits'
            get_coaddtile_geom('desoper', query, out_fname)
